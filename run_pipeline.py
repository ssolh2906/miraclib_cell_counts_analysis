...
from src.data.repository import (
    ...
    get_baseline_samples_per_project,
    get_baseline_subjects_per_response,
    get_baseline_subjects_per_sex,
    get_cell_counts,
)
...
from src.domain.subsets import combine_cohort_counts, filter_baseline_subset, summarize_population_means
...
def analyze_baseline_subset(conn: sqlite3.Connection) -> None:
    ...
    save_baseline_samples(get_baseline_miraclib_melanoma_pbmc_samples(conn))

    counts = combine_cohort_counts(
        get_baseline_samples_per_project(conn),
        get_baseline_subjects_per_response(conn),
        get_baseline_subjects_per_sex(conn),
    )
